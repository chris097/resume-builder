...
const ProfessionalSummary = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      console.log(values);
      setLoading(true);
      return fetchProfessionalSummary(
        values,
        (res: any) => {
          if (res.status === 201) {
  ...
  return (
    <div>
      <div className="max-w-5xl mx-auto w-full my-8">
        <button onClick={() => navigate(-1)} className="mb-6 text-basegray text-sm font-light flex space-x-1 cursor-pointer items-center">
          <MdOutlineKeyboardBackspace size="16px" /> <span>Back</span>
        </button>
        <div className="max-w-4xl mx-auto">
          ...
          <form onSubmit={formik.handleSubmit}>
            ...
            <div className="mt-3 text-basegray text-sm">
              ...
              <textarea id="bio" className="border w-full h-24 p-3 mt-1 focus:outline-none border-gray-300" {...formik.getFieldProps("bio")} />
            ...
            <div className="flex justify-between mt-16 font-light">
              <button type="button" className="border w-32 h-10">
                Back
              </button>
              {disableUser ? (
                <Button width="w-32" height="h-10" bg="bg-red-500" color="text-white" type="submit" name={loading ? "loading..." : "Sava & Next"} />
              ) : (
                <Button width="w-32" height="h-10" bg="bg-red-500" color="text-white" name={"Update"} />
              )}
            </div>
          </form>
        </div>
