...
const ResumeSection = () => {
  ...
  const { data } = useQueryApi("percentage", "user/percentage");

  console.log(data?.isUserInfo)

  const { logout } = useAuth();

  const navigate = useNavigate();

  const tabs: { tab: number; name: string, percentage: number, icon: any, has_data: any }[] = [
    {
      tab: 1, percentage: 20, has_data: true, name: "Personal Information", icon: <svg width="17" height="17" viewBox="0 0 17 17" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    {
      tab: 2, percentage: 40, has_data: true, name: "Experience", icon: <svg width="19" height="22" viewBox="0 0 19 22" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    {
      tab: 3, percentage: 60, has_data: true, name: "Technical Skills", icon: <svg width="18" height="19" viewBox="0 0 18 19" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    {
      tab: 4, percentage: 80, has_data: true, name: "Education", icon: <svg width="20" height="21" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    {
      tab: 5, percentage: 100, has_data: true, name: "Certification", icon: <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <div className="lg:w-[270px] bg-white h-screen overflow-auto pt-10">
            {tabs.map((tab, index) => (
              <button
                ...
                // disabled={data?.isUserInfo !== String(tabs[0].percentage)}
                className={`mt-3 ${
                  (
                    data?.isUserInfo === String(tabs[0].percentage) &&
                   data?.isUserExp === String(tabs[1].percentage) ||
                  data?.isSkills === String(tabs[2].percentage) &&
                  data?.isEducation === String(tabs[3].percentage) &&
                  data?.isCertificate === String(tabs[4].percentage)
                ) && (currentTab === index + 1)
                  ? "lg:bg-corered/20 font-medium" : ""
                } 
                lg:px-10 pr-5 cursor-pointer disabled:cursor-not-allowed items-center lg:gap-2 flex md:justify-start justify-center h-12 text-[#404653] text-sm w-[100px] lg:w-full`}
              >
                <div>{tab.icon}</div>
