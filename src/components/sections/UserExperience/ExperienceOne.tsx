...
const ExperienceOne = ({ data, months, years, isLoading, setLoading, id }: IProps) => {
    ...
    const startDate = data?.data[0]?.start_date;
    const endDate = data?.data[0]?.end_date;
    ...
    const initialValues: FormikProps = {
        ...
        start_month: "",
        end_month: "",
        start_year: data?.data[0]?.start_year || "",
        end_year: data?.data[0]?.end_year || ""
    }
    ...
    console.log(formik.values.start_month)

    return (
        <div>
            <p className='text-sm font-light mt-5'>#1</p>
            <form onSubmit={formik.handleSubmit}>
                    ...
                    <div className='w-full'>
                        ...
                        <div className='w-full h-[42px] flex items-center bg-white border border-[#E4E7EB] px-2 focus:outline-none bg-transparent text-basegray text-sm mt-1'>
                            <select onChange={formik.handleChange} name='start_month' className='w-full appearance-none text-xs h-full outline-none'>
                                ...
                                {months.map((month: any) => <option key={month.id}>{month.month}</option>)}
                            </select>
                ...
                <button
                    className="bg-corered text-white hover:bg-black/5 text-[13px] font-opensans w-full py-2 mt-5 mb-7"
                    type="submit">{isLoading ? "Loading" : (!data?.data?.length ? "Submit" : "Update")}</button>
            </form>
        </div>
    );
};
